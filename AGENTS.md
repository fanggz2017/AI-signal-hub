...
- **缓存**: Redis (使用 `ioredis`)。
- **验证**: 使用 [Zod](https://zod.dev/) 进行输入验证及模式定义，配合 `@hono/zod-validator` 中间件。
- **定时任务**: 使用 `node-cron` 处理计划任务 (如 GitHub 数据同步)。
...
- **HTTP 客户端**: Axios (封装了双 Token 无感刷新、请求队列和全局错误处理)。
...
8.  **测试规范**:
    - 测试用例需根据代码架构放置在就近的 `__test__` 或 `__tests__` 目录中。
    - 后端 Service 测试放于 `src/services/__test__/`。
    - 前端组件/Hook 测试放于特性目录下的 `__tests__/`。
    - 工具函数测试放于 `src/utils/__tests__/`。
